...
DROP POLICY IF EXISTS "Service role only access" ON auth_otp_sessions;
...
BEGIN
  RETURN QUERY
  WITH total_users AS (
    SELECT COUNT(*) as total FROM user_profiles WHERE total > 0
  ),
  method_counts AS (
    SELECT
  ...
  SELECT
    mc.method,
    mc.count,
    ROUND((mc.count::NUMERIC / NULLIF(tu.total, 0) * 100), 2) as pct
...
SET
  primary_auth_method = CASE
    ...
    WHEN email LIKE '%@temp.ivoire.ci' THEN 'phone'
    ELSE 'email'
  END,
  auth_methods_used = ARRAY[
    CASE
...
AND (mobile_money_operator IS NOT NULL OR email LIKE '%@mobilemoney.local' OR email LIKE '%@temp.ivoire.ci');
...
SELECT
  primary_auth_method,
  COUNT(*) as user_count,
  ROUND(COUNT(*)::NUMERIC / NULLIF((SELECT COUNT(*) FROM user_profiles), 0) * 100, 2) as percentage,
  ...
  COUNT(*) FILTER (WHERE email NOT LIKE '%@mobilemoney.local' AND email NOT LIKE '%@temp.ivoire.ci') as email_users
FROM user_profiles
...
COMMENT ON COLUMN user_profiles.primary_auth_method IS 'Méthode principale : mobile_money, whatsapp, email, google, phone';
